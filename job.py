...
class Job:
    def __init__(self, func: Callable, start_at: int = 0, max_working_time: int = 0, tries: int = 0):
        self.tries = tries
        self.func = func
    ...
    def run() -> Generator:
        ...
        while True:
            try:
                ...
                logger.info('Processing task "%(task_name)s"', {"task_name": task_name})
                if start_at:
                    logger.info("Task starts in %(start_at)s seconds", {"start_at": start_at})
                    time.sleep(start_at)
                if max_working_time:
