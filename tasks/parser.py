...
class Parser:
    ...
    def get_all_arcticles_ids(slug, certain_page=None):
        ...
        for page in range(1, 51):
            if certain_page:
                page = certain_page
            url = f"https://habr.com/ru/search/page{page}/?q={quote(slug)}&target_type=posts&order=relevance"
            try:
    ...
    def get_author_dict(bs_str):
        ...
        try:
            ...
            author_nickname = author_nickname.split('"alias":"', maxsplit=1)[1]
            ...
            author_dict["link"] = f"https://habr.com/ru/users/{author_nickname}/posts/"

            if '"speciality":"' in author_info:
                author_speciality = author_info
                author_speciality = author_speciality.split('"speciality":"', maxsplit=1)[1]
                author_speciality = author_speciality.split('\","', maxsplit=1)[0]
    ...
    def get_one_arcticle_info(article_id):
        ...
        try:
            time_published = bs_str.split('"timePublished":"', maxsplit=1)[1]
        except IndexError:
            time_published = time_published.split('"timePublished":"', maxsplit=1)[0]
    ...
    def main_parser(slug, certain_page):
        ...
        try:
            all_arcticles_ids = Parser.get_all_arcticles_ids(slug, certain_page)
            ...
            json_response["error_message"] = ex
    ...
    def start_parser():
        ...
        answer = ""
        while answer != "no":
            slug = input("Введите ключевое слово или слова для поиска на Habr.com \n")
            certain_page = -1
            while certain_page not in range(51):
                try:
                    certain_page = int(input(
                        "Введите номер страницы, информацию с которой хотите получить (от 1 до 50) \n"
                        "Или введите 0, чтобы получить информацию со всех страниц \n"
                    ))
                except Exception:
